...
export default function Terminal() {
  ...
  const [stage, setStage] = useState(0); // 0 idle, 1 typed cmd, 2 typed status, 3 show json+chart
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-border bg-white/[0.02] px-4 py-3">
        ...
        <span className="ml-2 font-mono text-xs text-ink-faint">status.sh</span>
      </div>
      <div className="min-h-[230px] p-6 font-mono text-[0.86rem] leading-[1.9]">
        ...
        {stage >= 1 && (
          <div className="text-ink-dim">
            Resolving profile…{stage === 1 && <span className="caret" />}
          </div>
        )}
        {stage >= 3 && (
          <>
...
{`{
  ...
  `}<span className="text-amber">&quot;leading&quot;</span>: <span className="text-cobalt">true</span>{`
            ...
            <div className="mt-4 text-xs text-ink-faint">// organic visibility, trending up</div>
